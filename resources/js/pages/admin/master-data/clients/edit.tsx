...
export default function ClientsEdit({ client, provinces, currentPage }: Props) {
    const [areas, setAreas] = useState<Area[]>([]);
    const [loadingAreas, setLoadingAreas] = useState(false);

    const { data, setData, put, processing, errors } = useForm({
        code: client.code,
        province_id: client.province_id.toString(),
        area_id: client.area_id?.toString() || '',
        cutoff_time: client.cutoff_time || '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`${clientRoutes.update.url(client.id)}?page=${currentPage}`);
    };
            ...
            <div className="space-y-6">
                ...
                <div className="flex items-center gap-4">
                    <Button variant="outline" size="icon" asChild>
                        <Link href={`${clientRoutes.index.url()}?page=${currentPage}`}>
